...
\begin{tikzpicture}[scale=1]
	...
	\node[dot=\circ_size, fill=red,draw] at (9.5*\fret,1*\h) {{\large \textbf{R}}};
	\node[dot=\circ_size, fill=yellow,draw] at (7.5*\fret,2*\h) {{\normalsize m3}};
	\node[dot=\circ_size, fill=yellow,draw] at (6.5*\fret,3*\h) {{\large 5}};
	\node[dot=\circ_size, fill=yellow,draw] at (9.5*\fret,3*\h) {{\large b7}};
	\node[dot=\circ_size, fill=yellow,draw] at (10.5*\fret,4*\h) {{\large m3}};
